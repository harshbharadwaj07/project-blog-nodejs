...
// mongoose.connect("mongodb+srv://admin-harsh:adminharsh6009@cluster0.pqd8uax.mongodb.net/todolistDB");
...
const day = date.getDate();
  ...
  Item.find({userno:req.user._id},function(err,foundItems){
    ...
    // }else{
      ...
      res.render("list", {user:req.user.username,listTitle: day, newListItems: foundItems});
    // }
  });
});
  ...
  List.findOne({"items.userno":req.user._id,listname:customList},function(err,result){
    if(err){
      ...
      if(!result){
        ...
        //Show existing list
        res.render("list",{user:req.user.username,listTitle: result.listname, newListItems: result.items});
      }
    }
  });
